...
class TextBasedSOPostAPIRecognition(SOPostAPIRecognition):
    ...
    def recognize(self, post: SOPost, is_completed=False):
        ...
        if is_completed:
            ...
            decode_clean_text2 = re.sub(self.pattern, " ",
                                        decode_clean_text2.replace('\n', ' ').replace(u'\u00a0', " "))
    ...
    def extract_api_from_sentence(self, sentence: str, is_title=False, is_completed=False):
        ...
        for word in words:
            ...
            if word.endswith("()"):
                oriApiSet.add(word)
                continue
            if word.startswith("(") or word.endswith(")") or word.startswith("www") or word.endswith("com"):
                continue
        ...
        if is_completed:
            ...
            for api in oriApiSet:
                ...
                if len(completed_api.split(".")) == 1 and len(completed_api.split("(")) == 1:
                    continue
                ori_api_2_linked_api_dic[api] = completed_api
                # oriApiSet_new.add
            return ori_api_2_linked_api_dic
        return oriApiSet
...
if __name__ == '__main__':
    ...
    text = "The standard W3C DOM API that you use was intended to have a common API between Java, C++ and JavaScript IIRC, so it's very complicated to use directly, as you have noticed."
